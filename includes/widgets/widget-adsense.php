<?php class AdsenseWidget extends WP_Widget
{
	function AdsenseWidget(){
		$widget_ops = array( 'description' => 'Exibe publicidade do AdSense' );
	...
	function form( $instance ){
		...
		# Adsense Code
		echo '<p><label for="' . $this->get_field_id('adsenseCode') . '">' . 'Código do AdSense:' . '</label><textarea cols="20" rows="12" class="widefat" id="' . $this->get_field_id('adsenseCode') . '" name="' . $this->get_field_name('adsenseCode') . '" >'. $adsenseCode .'</textarea></p>';	
	}
