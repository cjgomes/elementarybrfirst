...
if ( ! function_exists( 'get_thumbnail' ) ){
	function get_thumbnail($width=100, $height=100, $class='', $alttext='', $titletext='', $fullpath=false, $custom_field='', $post='')
	{
		...
		if ($thumb_array['thumb'] == '') {
			if ($custom_field == '') $thumb_array['thumb'] = esc_attr( get_post_meta($post->ID, 'Miniatura', $single = true) );
			else {
				$thumb_array['thumb'] = esc_attr( get_post_meta($post->ID, $custom_field, $single = true) );
				if ($thumb_array['thumb'] == '') $thumb_array['thumb'] = esc_attr( get_post_meta($post->ID, 'Miniatura', $single = true) );
			}
			...
			if ($fullpath) {
				...
				elseif ( $custom_field <> '' && get_post_meta($post->ID, 'Miniatura', $single = true) ) $thumb_array['fullpath'] = apply_filters( 'et_fullpath', et_path_reltoabs(esc_attr(get_post_meta($post->ID, 'Miniatura', $single = true))) );
			}
		}
	...
	function show_page_menu($customClass = 'nav clearfix', $addUlContainer = true, $addHomeLink = true){
		...
		if ($addUlContainer) echo('<ul class="'.$customClass.'">');
			if (et_get_option($shortname . '_home_link') == 'on' && $addHomeLink) { ?> 
				<li <?php if (is_front_page() || is_home()) echo('class="current_page_item"') ?>><a href="<?php echo esc_url( home_url() ); ?>"><?php _e('Página Inicial',$themename); ?></a></li>
			<?php };
...
function head_addons(){
	...
	if ( ! function_exists( 'get_custom_header' ) ){
		...
		echo '<meta content="' . esc_attr( $theme_info->display('Name') . ' v.' . $theme_info->display('Versão') ) . '" name="generator"/>';
	}
...
if ( ! function_exists( 'elegant_titles' ) ){
	function elegant_titles() {
		...
		if (is_single() || is_page()) { 
			...
			$exists3 = get_post_meta($postid, ''.$key.'', true);
					if (et_get_option($shortname.'_seo_single_title') == 'on' && $exists3 !== '' ) echo $exists3; 
						...
						if ( $seo_single_type == 'Site | Notícia' ) echo $sitename . $seo_single_separate . wp_title('',false,''); 
						if ( $seo_single_type == 'Notícia | Site' ) echo wp_title('',false,'') . $seo_single_separate . $sitename;
						if ( $seo_single_type == 'Notícia' ) echo wp_title('',false,'');
					}
		...
		if (is_category() || is_archive() || is_search()) {
			...
			if ( $seo_index_type == 'Site | Categoria' ) echo $sitename . $seo_index_separate . wp_title('',false,''); 
			if ( $seo_index_type == 'Categoria | Site') echo wp_title('',false,'') . $seo_index_separate . $sitename;
			if ( $seo_index_type == 'Categora') echo wp_title('',false,'');
		}	  
	}
}
...
if ( ! function_exists( 'et_resize_image' ) ){
	function et_resize_image( $thumb, $new_width, $new_height, $crop ){
		...
		if ( !$size ) return new WP_Error('invalid_image_path', __('Imagem não existe'), $thumb);
...
function et_check_themes_updates( $update_transient ){
	...
	$theme_request = wp_remote_post( '', $options );
...
function et_admin_update_theme_message( $default_translated_text, $original_text, $domain ) {
	global $themename;
    $theme_page_message = 'Há uma nova versão do framework disponível. Contacte o gestor do tema para efetuar a atualização corretamente.';
	$updates_page_message = 'Atualização não disponível.';

    if ( is_admin() && $original_text === $theme_page_message ) {
        return __( 'Há uma nova versão do framework disponível. Contacte o gestor do tema para efetuar a atualização corretamente.', $themename );
    }
