...
if ( ! function_exists( 'et_epanel_admin_js' ) ){
	function et_epanel_admin_js(){
		...
		wp_localize_script( 'epanel_functions_init', 'ePanelSettings', array(
			...
			'et_saving_text' => esc_html__( 'Salvando', $themename ),
			'et_options_saved_text' => esc_html__( 'Salvo', $themename ),
...
function et_add_epanel() {
    ...
    $core_page = add_theme_page( $themename . ' ' . esc_html__( '', $themename ), $themename . ' ' . esc_html__( 'Opções do Layout', $themename ), 'switch_themes', basename(__FILE__), 'et_build_epanel' );
...
if ( ! function_exists( 'et_build_epanel' ) ){
	function et_build_epanel() {
		...
		if ( isset($_GET['reset']) ) {
			if ( $_GET['reset'] ) echo '<div id="message" class="updated fade"><p><strong>' . esc_html( $themename ) . ' ' . esc_html__( 'Configurações resetadas.', $themename ) . '</strong></p></div>';
		}
	?>
							...
							<div id="epanel-content">
								...
								<ul id="epanel-mainmenu">
									<?php if(in_array('general',$epanelMainTabs)) { ?>
										<li><a href="#wrap-general"><img src="<?php echo get_template_directory_uri() ?>/epanel/images/general-icon.png" class="pngfix" alt="" /><?php esc_html_e( 'Opções Gerais', $themename ); ?></a></li>
									<?php }; ?>
									<?php if(in_array('layout',$epanelMainTabs)) { ?>
										<li><a href="#wrap-layout"><img src="<?php echo get_template_directory_uri() ?>/epanel/images/layout-icon.png" class="pngfix" alt="" /><?php esc_html_e( 'Layout', $themename ); ?></a></li>
									<?php }; ?>
		...
		<?php
						...
						<div class="box-content">
							...
							<?php } elseif ( 'upload' == $value['type'] ) { ?>
								...
								<div class="upload_buttons">
									<span class="upload_image_reset"><?php esc_html_e( 'Resetar', $themename ); ?></span>
									<input class="upload_image_button" type="button" value="<?php esc_attr_e( 'Enviar imagem', $themename ); ?>" />
								</div>
							...
							<?php } elseif ( 'checkboxes' == $value['type'] ) {
								...
								if ( empty( $value['options'] ) ) {
									esc_html_e( "Você não tem páginas criadas", $themename );
								} else {
									$i = 1;
									$className = 'inputs';
			...
			<?php } //end foreach ($options as $value) ?>
				...
				<div id="epanel-bottom">
					...
					<input name="save" type="submit" value="<?php esc_html_e( 'Salvar mudanças', $themename ); ?>" id="epanel-save" />
			...
			<div style="position: relative;">
				<div class="defaults-hover">
					<?php _e( 'Isso apagará todas as configurações já criadas. <strong>Tem certeza que quer isso?', $themename ); ?></strong>
					<div class="clearfix"></div>
					<form method="post">
						<?php wp_nonce_field( 'et-nojs-reset_epanel', '_wpnonce_reset' ); ?>
						<input name="reset" type="submit" value="<?php esc_html_e( 'Resetar', $themename ); ?>" id="epanel-reset" />
		...
		<div id="epanel-ajax-saving">
			...
			<span><?php esc_html_e( 'Salvando...', $themename ); ?></span>
		</div>
	...
	function epanel_save_data( $source ){
		...
		if ( isset($_POST['action']) ) {
			if ( 'save_epanel' == $_POST['action'] ) {
				...
				header("Localização: themes.php?page=$epanel&reset=true");
				die('1');
			}
		}
	}
